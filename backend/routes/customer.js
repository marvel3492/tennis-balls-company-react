...
import { adminOnly, renderError, renderAllRecords, renderOneRecord, deleteRecord, guestOnly, guestOrAdmin, adminOrCustomer } from '../util.js';
var router = Router();
...
router.get('/credentials', function(req, res, _next) {
    res.json({customer_id: req.session.customer_id, custname: req.session.custname, isadmin: req.session.isadmin});
});
            ...
            if (result[0]) {
                ...
                compare(req.body.password, result[0].password, function(err, result2) {
                    if (err) {
                        ...
                        res.status(401).json({success: true, customer_id: req.session.customer_id, custname: req.session.custname, isadmin: req.session.isadmin});
                    } else {
                ...
                res.status(401).json({success: false, message: "Wrong Username"});
            }
        }
    });
});
