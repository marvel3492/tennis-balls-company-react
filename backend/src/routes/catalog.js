...
import { getCartQuantity, isCartEmpty } from '../../../shared/src/Cart.js';
import { isCatalogArray, isProductArray } from '../../../shared/src/Data.js';
import db from '../db.js';
import { adminOrCustomer, handleSession, renderAllRecords, renderError } from '../utils.js';
const router = Router();
 ...
 * @returns {asserts req is ExpressRequest & { session: Session & { cart: CartType } }}
 */
...
// ==================================================
router.post('/add', handleSession(function(req, res, _next) {
    createCart(req);
...
// ==================================================
router.get('/cart', handleSession(function(req, res, _next) {
    createCart(req);
    if (isCartEmpty(req.session.cart)) {
        ...
        db.all(query, Object.keys(req.session.cart), (err, result) => {
            if (err) {
                ...
                let totprice = 0;
                let totqty = 0;
                const lineItemCosts = /** @type number[] */ ([]);
                result.forEach((item) => {
                    if (!req.session.cart) {
                        renderError(res, new Error("Cart is undefined"));
                    } else {
...
// ==================================================
router.delete('/remove', handleSession(function(req, res, _next) {
    if (req.session.cart) {
...
// ==================================================
router.post('/checkout', adminOrCustomer, handleSession(function(req, res, _next) {
    if (!req.session.cart || isCartEmpty(req.session.cart)) {
        res.json({ordernum: -1});
    } else {
        db.serialize(() => {
            let error = /** @type OptionalType<Error> */ (null);
                        ...
                        for (const [key, value] of Object.entries(req.session.cart)) {
                            ...
                            db.all(query, key, (err, result) => {
                                if (err) {
                                    ...
                                    setError(new Error("Unexpected data type"));
                                } else if (!result[0]) {
                                    setError(new Error("Product not found"));
                                } else {
                                    // Perform ORDERDETAIL table insert
                                    db.run("INSERT INTO orderdetail(order_id, product_id, saleprice, qty) VALUES (?, ?, (SELECT saleprice FROM product WHERE product_id = ?), ?)", [order_id, key, result[0].saleprice, value], setError);
                                    db.run("UPDATE product SET stock = stock - ? WHERE product_id = ?", [value, key], setError);
                                }
                            });
                        }
                    } else {
                        setError(new Error("Empty cart"));
                    }
