...
CREATE TABLE IF NOT EXISTS product (
    ...
    prodimage TEXT NOT NULL CHECK (LENGTH(prodimage) >= 1 AND LENGTH(prodimage) <= 50),  -- Use the file names found in the public\images folder
    description TEXT NOT NULL DEFAULT '' CHECK (LENGTH(description) <= 500),
    saleprice DECIMAL(8, 2) NOT NULL DEFAULT 0 CHECK (saleprice >= 0),
...
CREATE TABLE IF NOT EXISTS promotion (
    ...
    promoimage TEXT NOT NULL CHECK (LENGTH(promoimage) >= 1 AND LENGTH(promoimage) <= 20),  -- Use the file names found in the public\images folder
    description TEXT NOT NULL DEFAULT '' CHECK (LENGTH(description) <= 200),
    startdate DATE NOT NULL,
    enddate DATE NOT NULL,
    discountrate DECIMAL(5, 2) NOT NULL DEFAULT 0 CHECK (discountrate >= 0)  -- As a percentage (10%, 0.5%, etc.)
);